...
function IntraDay({ isWsLoaded, tableType }) {
	...
	const blotterProps = {
		...
		openTable: openTable,
		isWsLoaded,
		columns: [{ name: 'currency' }, { name: 'last_price' }, { name: 'net_delta' }, { name: 'open_delta' }, { name: 'trade_delta' }],
	}
	console.log(`RENDERING ${tableType}!`, `Open table ${tableType} ${openTable}`)
	return (
		<>
			{isWsLoaded && <Button onClick={handleOpen}>{`Open Intraday Monitor`}</Button>}
