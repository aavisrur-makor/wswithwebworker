export const receiveBlotterData = (message, cb, id) => {
	if (message.data && message.data.type === 'pnl_monitor' && message.data.content) {
	...
	if (message.data && message.data.type === 'subscription' && message.data.content && message.data.id === id) {
		cb(message.data.content)
	}
}
